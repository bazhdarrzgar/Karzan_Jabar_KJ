...
export default function Trade() {
  ...
  const renderAccountCard = (accountInfo: AccountInfo) => {
    ...
    return (
      <TabsContent key={accountInfo.id} value={accountInfo.id} className="space-y-4 sm:space-y-6 relative z-10">
        {/* Account Info Card */}
        <Card className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl border-blue-200 dark:border-blue-800 shadow-xl">
          ...
          <CardContent className="pt-0">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
              <div className="bg-white/50 dark:bg-gray-900/50 backdrop-blur p-3 sm:p-4 rounded-lg shadow-sm border border-gray-100 dark:border-gray-800">
              ...
              <div className="bg-white/50 dark:bg-gray-900/50 backdrop-blur p-3 sm:p-4 rounded-lg shadow-sm sm:col-span-2 lg:col-span-1 border border-gray-100 dark:border-gray-800">
        ...
        {/* Stats Cards */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
          <Card className="border-blue-200 bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl shadow-lg">
          ...
          <Card className={`${stats.totalProfit >= 0 ? 'border-green-200 bg-white/80 dark:bg-gray-800/80' : 'border-red-200 bg-white/80 dark:bg-gray-800/80'} backdrop-blur-xl shadow-lg`}>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <Card className="border-green-200 bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl shadow-lg sm:col-span-2 lg:col-span-1">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
          ...
          <Card className="overflow-hidden bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl shadow-lg">
            <CardHeader className="pb-3 sm:pb-6">
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-slate-50 dark:bg-gray-900">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600 dark:text-gray-400 font-medium">{t('loading_trading_data')}</p>
        </div>
      </div>
    );
  }
      ...
      <div className="container mx-auto px-3 sm:px-4 lg:px-6 py-4 sm:py-6 lg:py-8 max-w-7xl relative z-10">
        <div className="mb-6 sm:mb-8">
          <div className="flex items-center gap-2 sm:gap-4 mb-3 sm:mb-4">
            <a href={`/${i18n.language}`} className="bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm border border-gray-200 dark:border-gray-700 p-2 rounded-xl text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 flex items-center gap-1 sm:gap-2 transition-all shadow-sm hover:shadow-md text-sm sm:text-base">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 sm:h-5 sm:w-5" viewBox="0 0 20 20" fill="currentColor">
                <path fillRule="evenodd" d="M9.707 16.707a1 1 0 01-1.414 0l-6-6a1 1 0 010-1.414l6-6a1 1 0 011.414 1.414L5.414 9H17a1 1 0 110 2H5.414l4.293 4.293a1 1 0 010 1.414z" clipRule="evenodd" />
              </svg>
              <span className="hidden xs:inline font-medium">{t('back_to_home')}</span>
              <span className="xs:hidden font-medium">{t('back')}</span>
            </a>
          </div>
            ...
            <div>
              <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-gray-900 dark:text-white mb-2 tracking-tight">
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-purple-600">
                  {t('trade_dashboard_title')}
                </span>
              </h1>
              <p className="text-base sm:text-lg text-gray-600 dark:text-gray-400 max-w-2xl font-medium">{t('trade_dashboard_desc')}</p>
            </div>
          </div>
        </div>
        ...
        <Tabs defaultValue={accountsInfo[0].id} className="w-full">
          ...
          <div className="mb-6 sm:mb-8">
            <TabsList className="w-full grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 h-auto bg-transparent p-0">
              {accountsInfo.map(account => {
                ...
                return (
                  <TabsTrigger
                    key={account.id}
                    value={account.id}
                    className="flex flex-col sm:flex-row items-center justify-between gap-2 py-4 px-6 bg-white/50 dark:bg-gray-800/50 backdrop-blur-xl border border-gray-200 dark:border-gray-700 rounded-2xl data-[state=active]:bg-blue-600 data-[state=active]:text-white data-[state=active]:border-blue-600 transition-all shadow-sm hover:shadow-md group"
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-blue-100 dark:bg-blue-900/50 flex items-center justify-center group-data-[state=active]:bg-white/20">
                        <Activity className="w-5 h-5 text-blue-600 dark:text-blue-400 group-data-[state=active]:text-white" />
                      </div>
                    ...
                    <Badge
                      ...
                      className={`text-xs font-bold px-3 py-1 rounded-full ${stats.totalProfit >= 0 ? 'bg-green-500 hover:bg-green-600' : 'bg-red-500 hover:bg-red-600'} group-data-[state=active]:bg-white group-data-[state=active]:text-blue-600`}
                    >
                      ${stats.totalProfit.toFixed(2)}
                    </Badge>
                  </TabsTrigger>
